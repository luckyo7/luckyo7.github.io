...
(() => {
  // Per-page store: one shared sidecar hit a ~2MB write ceiling once several
  // projects had photos, so each page gets its own file. Detail pages share
  // one HTML file (detail.dc.html) differentiated only by ?p=<slug>, so key
  // those by the slug; every other page keys by its filename.
  const STATE_FILE = (() => {
    try {
